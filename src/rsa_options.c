...
static void help(char** argv)
{
    ...
    write(STDOUT_FILENO, "-inform\t\tInput format\n", 22);
...
static t_rsa_options read_options(char* argv[], int argc)
{
    ...
    while(i < argc)
    {
        ...
        else if(!ft_strcmp(*(argv + i), "-inform") && !ret.password_out)
        {
            if(*(argv + i + 1))
            {
                if(ft_strcmp(*(argv + i + 1), "PEM"))
                {
                    error_message(argv[1], *(argv + i), "' only PEM format is implemented\n");
                    exit(1);
                }
            }
            else
        ...
        else if(!ft_strcmp(*(argv + i), "-outform") && !ret.password_out)
        {
            if(*(argv + i + 1))
            {
                if(ft_strcmp(*(argv + i + 1), "PEM"))
...
void rsa_options(char** argv, int argc)
{
        ...
        if(ask.options & 1)
            ...
            vis = PRIVATE;
        size = parse_pem(&pem, buff, size, vis, &salted, &s_iv);
        if(pem)
        {
            if(salted)
            {
            ...
            if((ask.options & 0b1000) == 0b1000)
            {
                if(ask.options & 1)
                {
                    error_message(argv[1], NULL, ": Only private keys can be checked\n");
                    exit(1);
                }
                else
                    check_rsakey(k);
            ...
            if((ask.options & 0b10000) == 0b10000)
                print_modulus(1, k);
            if((ask.options & 0b1000001) == 0b1000000)
            {
                size_t len;
                if(!ask.password_out)
                {
                    len = ft_getpass(&password_out, 0, "des");
                ...
                DK = pbdkf2((uint8_t*)ask.password_out, len, salt, 10000, 24, sha256, 32);
                for (size_t i = 0; i < 16; i++)
                    key[i] = DK[i];
                free(s_iv);
            ...
            if((ask.options & 0b100000) == 0)
            {
                ...
                {
                    uint8_t* pb = format_der(der, PUBLIC);
                    char* text = base64_encode(pb, der->size);
                    print_rsakey(fd, text, PUBLIC, salted, s_iv);
                    free(text);
                    free(pb);
                }
                else
